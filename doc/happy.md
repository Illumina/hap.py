...
If an input file has the extension .gvcf or .gvcf.gz, Hap.py assumes that
it is a genome VCF (GVCF) that contains depth information about every position
in the genome in addition to the variant locations. The default behavior is
to first convert the GVCF to a VCF by stripping out non-variant loci as 
well as extraneous field information. Currently this has been tested for
GVCFs generated by the DRAGEN software toolchains.

Setting this flag will override the default behavior and prevent Hap.py from 
converting files ending in .gvcf or .gvcf.gz. 

...
if [[ ! -z $BUILD_VCFEVAL ]]; then
    if [[ ! -d ${ISD}/libexec/rtg-tools-install ]]; then
        ...
        cd rtg-tools
        wget http://github.com/RealTimeGenomics/rtg-tools/archive/3.8.2.tar.gz -O ${TLD}/rtg-tools/rtg-tools.tar.gz
        ...
        RTG_ZIPFILE=$(ls rtg-tools-3.8.2/dist/*-nojre.zip | head -1)

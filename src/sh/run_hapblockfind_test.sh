...
${HCDIR}/hapblockfind ${IF}.gz:NA12877 ${IF}.gz:NA12878 \
...
if [[ -f "$HG19" ]]; then
	...
	${HCDIR}/hapblockfind ${DIR}/../../example/PG_performance.vcf.gz ${DIR}/../../example/performance.varsonly.vcf.gz \
	...
	${HCDIR}/bcftools view -H ${DIR}/../../example/PG_performance.vcf.gz chr1 | grep -v '0|0' | grep -v '0/0' > ${TF_X1}
	${HCDIR}/bcftools view -H ${DIR}/../../example/PG_performance.vcf.gz -R ${TF_r} chr1 | grep -v '0|0' | grep -v '0/0' > ${TF_X2}
	...
	TF_X1=`mktemp -t hapblockfind.XXXXXXXXXX` || exit 1
	TF_X2=`mktemp -t hapblockfind.XXXXXXXXXX` || exit 1
	${HCDIR}/bcftools view -H ${DIR}/../../example/performance.varsonly.vcf.gz chr1 | grep -v '0|0' | grep -v '0/0' > ${TF_X1}
	${HCDIR}/bcftools view -H ${DIR}/../../example/performance.varsonly.vcf.gz -R ${TF_r} chr1 | grep -v '0|0' | grep -v '0/0' > ${TF_X2}
